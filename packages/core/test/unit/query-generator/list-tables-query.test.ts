...
describe('QueryGenerator#listTablesQuery', () => {
  ...
  it('produces a query that lists all tables', () => {
    expectsql(() => queryGenerator.listTablesQuery(), {
      ...
      hana: `SELECT TABLE_NAME AS "tableName", SCHEMA_NAME AS "schema" FROM SYS.TABLES WHERE SCHEMA_NAME NOT LIKE '\\_SYS%' ESCAPE '\\' AND SCHEMA_NAME NOT LIKE '\\_SAP%' ESCAPE '\\' AND SCHEMA_NAME NOT IN ('PUBLIC', 'SYS', 'PAL_CONTENT', 'PAL_ML_TRACK', 'PAL_ANNS_CONTENT', 'PAL_STEM_TFIDF', 'SAP_PA_APL', 'BROKER_PO_USER', 'BROKER_USER') ORDER BY SCHEMA_NAME, TABLE_NAME`,
    });
  });
